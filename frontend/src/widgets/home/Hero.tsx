...
import { useFeaturedProducts } from '@/features/manage-product'
                ...
                <CarouselContent className="h-full">
                  {featuredProducts.map((product: Product) => {
                    const defaultVariant =
                      product.variants?.find((v: ProductVariant) => v.isDefault) ??
                      product.variants?.[0]
                    const primaryImage =
                      product.productImages?.find((img: { isPrimary: boolean }) => img.isPrimary) ??
                    ...
                    const variantMsrps =
                      product.variants
                        ?.map((v: ProductVariant) => Number(v.msrp))
                        .filter((m: number) => m > 0) ?? []
                    const highestMsrp =
